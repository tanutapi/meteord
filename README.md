[![Circle CI](https://circleci.com/gh/tanutapi/meteord/tree/master.svg?style=svg)](https://circleci.com/gh/tanutapi/meteord/tree/master)
...
* `node-8-base`, `node-8.17.0-base`
* `node-8-binbuild`, `node-8.17.0-binbuild`
* `node-8-onbuild`, `node-8.17.0-onbuild`
* `node-8-devbuild`, `node-8.17.0-devbuild`

### Older Node versions

For brevity, not all possibilities are listed above and there are many more available.  It's recommended that you use the latest version within the series which your Meteor was designed for (see titles above).  The most recent version will be tagged with a `node-x-*` tag accordingly.  For the full list, please see the ["Tags" tab](https://hub.docker.com/r/tanutapi/meteord/tags/) above.
...
For that, you can use `tanutapi/meteord:onbuild` as your base image. Magically, that's only thing you have to do. Here's how to do it:

Add following `Dockerfile` into the root of your app:

~~~shell
FROM tanutapi/meteord:onbuild
~~~
...
> WARNING: Don't use `tanutapi/meteord:devbuild` for your final build. If you used it, your image will carry the Meteor distribution as well. As a result of that, you'll end up with an image with ~700 MB.
