...
if [[ $REBUILD_NPM_MODULES ]]; then
  echo "=> tanutapi/meteord:bin-build is NOT TESTED AT ALL (and maybe not necessary???)"
  ...
  echo "     discourage use of the :bin-build image if possible!  Thanks! -tanutapi"
  if [ -f /opt/meteord/rebuild_npm_modules.sh ]; then
    cd programs/server
    bash /opt/meteord/rebuild_npm_modules.sh
    cd ../../
  else
    echo "=> Use tanutapi/meteord:bin-build for binary bulding."
    exit 1
  fi
fi
